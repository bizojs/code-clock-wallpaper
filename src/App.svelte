...
<div class="flex w-screen h-screen relative">
    ...
    {#if $backgroundImage?.value ?? false}
        <img
            ...
            src="wallpaper.jpg"
            alt=""
            class="z-[-2] fixed top-0 left-0 w-full h-full select-none pointer-events-none"
        >
        <div class={cn(
    ...
    {#if !hidden}
        <div
            transition:fly={{ y: -60, duration: 800, easing: cubicInOut }}
            class="flex flex-col w-fit fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-[999]"
        >
